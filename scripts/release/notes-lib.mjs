...
function sanitizeTitle(title) {
  return String(title).replaceAll('[', '(').replaceAll(']', ')').trim();
}
  ...
  for (const commit of commits) {
    ...
    for (const pull of pulls) {
      ...
      pullRequestByNumber.set(pull.number, {
        number: pull.number,
        title: sanitizeTitle(pull.title),
        url: pull.html_url,
...
export function renderPullRequestBullets(pullRequests) {
  if (pullRequests.length === 0) {
    return '- _No merged pull requests in this range._';
  }

  return pullRequests
    .map((pullRequest) => `- [${pullRequest.title}] (#${pullRequest.number})`)
    .join('\n');
}
...
export function parseReleaseCliArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (argument === '--to-ref' || argument === '--to_ref' || argument === '--tag') {
